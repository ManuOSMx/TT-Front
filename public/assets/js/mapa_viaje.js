  ...
  map.events.add("ready", function () {
    ...
    map.layers.add(
      new atlas.layer.LineLayer(datasource, null, {
        ...
        strokeColor: [
          "to-color",
          ["at", ["get", "resultIndex"], ["literal", routeColors]],
        ],
        strokeWidth: 5,
        lineJoin: "round",
        lineCap: "round",
      ...
      .autocomplete({
        ...
        source: function (request, response) {
          ...
          var requestUrl = geocodeServiceUrlTemplate
            ...
            .replace(
              "{searchType}",
              document.querySelector('input[name="searchTypeGroup"]:checked')
                .value
            )
...
function calculateRoute() {
    ...
    postBody: {
      supportingPoints: {
        ...
        geometries: [
          ...
          new atlas.data.Point([-100.38806, 20.58806]), //A supporitng point on the on I90.
  ...
  if (options.maxAlternatives > 0) {
    ...
    options.alternativeType =
      altTypeElm.options[altTypeElm.selectedIndex].value;
  }
  ...
  routeURL
    ...
    .then((directions) => {
      ...
      var html = [
        "<table><tr><td>Ruta</td><td>Distancia</td><td>Tiempo</td></tr>",
      ];
      ...
      for (var i = 0; i < directions.routes.length; i++) {
        ...
        html.push(
          '<tr><td><div class="colorBlock" style="background-color:',
          routeColors[i],
          '"></div></td><td>',

          //Convert distance to meters and round to 1 decimal place.
          Math.round(
            (directions.routes[i].summary.lengthInMeters / 1000) * 10
          ) / 10,
          " km</td><td>",
